...
function preventImpossible() {
  while (
    newGridArray[9][9] == forbiddenColor ||
    newGridArray[9][8] == forbiddenColor
  ) {
    newGridArray = [];
    createNewGridArray();
    drawGrid();
...
function testAbs() {
  sortArray = [
    [Math.abs(xP + 1 - xSp) + Math.abs(yP - ySp)],
    [Math.abs(xP - 1 - xSp) + Math.abs(yP - ySp)],
    [Math.abs(xP - xSp) + Math.abs(yP - 1 - ySp)],
    [Math.abs(xP - xSp) + Math.abs(yP + 1 - ySp)]
  ];
